...
- 2026-07-03：Phase 6 入口预检发现并修复 live cutover 开关未贯通：CLI `snapshot-collector-run-once/backfill` 和 quality gate 原先仍硬编码只允许 `dragonboard_backend_shadow`，导致设置 `QUANT_BOARD_SNAPSHOT_COLLECTOR_ALLOW_LIVE_DATASET=1` 后仍不能对 `dragonboard_live` 做 dry-run。现已改为默认拒绝 live、显式 allow 时放行 `dragonboard_live`，任意其它 dataset 仍拒绝。验证：`tests/test_snapshot_collector_cli.py tests/test_snapshot_collector_quality_gate.py` 为 104 passed；真实 CLI `snapshot-collector-run-once --dataset-id dragonboard_live --snapshot-type half_hour --trading-date 2026-07-03 --slot-time 15:00 --dry-run` 在显式 allow 下返回 `status=dry_run`、`quality.ok=true`、`blockingIssues=[]`。
- 2026-07-03：按用户明确决策取消额外观察门槛，直接切换 backend snapshot collector 到 `dragonboard_live`。本机运行配置 `quant-board/.env.local` 已设置 `QUANT_BOARD_SNAPSHOT_COLLECTOR_ENABLED=1`、`QUANT_BOARD_SNAPSHOT_COLLECTOR_DATASET_ID=dragonboard_live`、`QUANT_BOARD_SNAPSHOT_COLLECTOR_ALLOW_LIVE_DATASET=1`；`8001 /api/snapshot-collector/scheduler/status` 返回 `enabled=true`、`running=true`、`dataset_id=dragonboard_live`、`snapshot_types=[half_hour,daily]`。Phase 6 状态更新为：正式 live collector 已切换，不再以额外连续观察天数作为进入/完成门槛；后续只按真实报错或审计发现处理具体缺陷。
