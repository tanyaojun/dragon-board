...
- SQLite 替换 IndexedDB 的当前切口：Dragon Board 正式聚合读口应优先走 QuantBoard `GET /api/snapshots/frames`；IndexedDB 只作为迁移源、缓存和远端不可用回退。
...
13. 逐步替换 IndexedDB 时必须先接入 SQLite 读接口并保留可控回退；确认迁移和行数校验完成前，不得删除浏览器历史数据或关闭回退。
