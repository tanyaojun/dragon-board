...
## 10. 设计原则（必读：2026-07-13 Backend Snapshot Collector 复盘教训）

**数据采集系统禁止设计质量阻断。** 采集器职责是保存数据，不是判断好坏。唯一的拒写条件：所有数据源全部不可用。其余情况一律保存并打标记。详见 `AGENTS.md` 第 11 节。

**新方案设计前必须逐条确认：**
- 核心目标能否 5 句话说清？
- 有没有质量阻断逻辑？→ 删掉
- 有没有分阶段验证？→ 删掉，直接生产小范围观察
- 有没有独立守护进程/计划任务？→ 集成到主进程
- 有没有新 API/CLI/集合？→ 问自己：没有这个能不能工作？
- 条件判断语义（任一/全部/至少）是否正确？
- 环境变量默认值是否生产安全？
- 第一个上线日有没有人工验证 MongoDB 实际入库数据？
