...
const dataSourceManager = {
  getActiveSource: () => 'tencent',
  getStatus: () => ({ tencent: 'active' }),
...
// ========== AKShare 数据代理 ==========
app.get('/api/akshare/fund-flow', async (req, res) => {
  try {
    const { type = 'rank' } = req.query

    // AKTools 默认运行在 8080 端口
    const baseUrl = 'http://127.0.0.1:8080'
    let url = ''

    if (type === 'rank') {
      url = `${baseUrl}/api/public/stock_individual_fund_flow_rank?indicator=今日`
    } else if (type === 'market') {
      url = `${baseUrl}/api/public/stock_market_fund_flow`
    } else if (type === 'sector') {
      url = `${baseUrl}/api/public/stock_sector_fund_flow_rank?indicator=今日&sector_type=2`
    }

    console.log(`[AKShare代理] 请求: ${url}`)
    ...
    console.error('[AKShare代理] 失败:', error.message)

    // 返回更详细的错误信息
    res.status(500).json({
      error: '获取资金流向失败',
      message: error.message,
      tip: '请确认 AKTools 是否在运行 (python -m aktools)',
    })
  }
})

// 获取A股实时行情
app.get('/api/akshare/spot', async (req, res) => {
  try {
    const url = 'http://127.0.0.1:8080/api/public/stock_zh_a_spot'
    const response = await axios.get(url, { timeout: 10000 })
    res.json(response.data)
  } catch (error) {
    console.error('[AKShare代理] 获取行情失败:', error)
    res.status(500).json({ error: error.message })
  }
})
...
app.get('/api/quotes/smart', async (req, res) => {
  try {
    ...
    const codeList = codes
      .split(',')
      .filter((code) => code && /^\d{6}$/.test(code.trim()))
      .map((code) => code.trim())
    ...
    console.log(`[智能路由] 开始处理请求，有效codes: ${codeList.length}`)

    // 2. 尝试东财
    try {
      const eastmoneyRes = await axios.get(
        `http://localhost:${PORT}/api/quotes/eastmoney?codes=${codeList.join(',')}`,
        { timeout: 5000 },
      )

      if (eastmoneyRes.data?.data?.diff?.length > 0) {
        const firstStock = eastmoneyRes.data.data.diff[0]
        if (firstStock.f62 !== 0 && firstStock.f62 !== undefined) {
          console.log('[智能路由] ✅ 使用东财数据')
          return res.json(eastmoneyRes.data)
        }
      }
    } catch (e) {
      console.log('[智能路由] 东财失败:', e.message)
    }

    // 3. 尝试腾讯
    try {
      const tencentRes = await axios.get(
        `http://localhost:${PORT}/api/quotes/tencent?codes=${codeList.join(',')}`,
        { timeout: 5000 },
      )

      if (tencentRes.data?.data?.diff?.length > 0) {
        console.log('[智能路由] ✅ 使用腾讯数据')
        return res.json(tencentRes.data)
      }
    } catch (e) {
      console.log('[智能路由] 腾讯失败:', e.message)
    }

    // 4. 全部失败，返回空数据而不是500
    res.json({ rc: 0, data: { diff: [] } })
  } catch (error) {
    console.error('[智能路由] 错误:', error.message)
    // 返回空数据，避免前端崩溃
    res.json({ rc: 0, data: { diff: [] } })
  }
})
