...
import fs from 'fs'
import { exec } from 'child_process' // ✅ 放在这里
...
app.get('/api/quotes/smart', async (req, res) => {
  try {
    ...
    console.log(`[智能路由] 开始处理请求，codes: ${codes.substring(0, 50)}...`)
    ...
    try {
      ...
      const eastmoneyRes = await axios.get(eastmoneyUrl, {
        timeout: 5000,
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        },
    ...
    try {
      ...
      const tencentRes = await axios.get(tencentUrl, {
        timeout: 5000,
      ...
      // 解析腾讯数据
      const text = iconv.decode(tencentRes.data, 'gbk')
      ...
      if (results.length > 0) {
        console.log(`[智能路由] ✅ 使用腾讯数据，${results.length} 条`)
        return res.json({
          rc: 0,
          data: { diff: results },
        })
...
app.post('/api/tdx/:entry', async (req, res) => {
  ...
  try {
    ...
    // 处理TDX返回的错误格式
    if (typeof response.data === 'string' && response.data.includes('E|-1|')) {
      console.log(`[TDX代理] TDX返回错误: ${response.data}`)

      // 根据不同entry返回模拟数据
      if (entry === 'HQServ.PBSdstat') {
        // 返回模拟的涨跌家数数据
        return res.json({
          data: {
            up: 1057,
    ...
    console.error(`[TDX代理] 失败:`, error.message)
...
app.get('/api/surge-stock/performance', async (req, res) => {
  ...
  console.log('请求头:', {
    ...
    cookie: req.headers['cookie'] ? '存在' : '无',
  })
...
// ========== 检查服务状态 ==========
app.get('/api/quick-status', (req, res) => {
...
app.listen(PORT, () => {
  ...
  console.log('   GET  /api/quotes?codes=...    - 批量行情（兼容）')
  console.log('   GET  /api/quotes/smart?codes=... - 智能行情')
  console.log('   GET  /api/quotes/tencent?codes=... - 腾讯行情')
  console.log('   GET  /api/quotes/eastmoney?codes=... - 东财行情')
  console.log('   GET  /api/quotes/sina?codes=... - 新浪行情')
...
app.get('/launcher.html', (req, res) => {
  const launcherPath = join(__dirname, 'launcher.html')
  if (fs.existsSync(launcherPath)) {
    res.sendFile(launcherPath)
  } else {
    res.status(404).send('启动管理器页面不存在，请先创建 launcher.html 文件')
  }
})
...
app.get('/test.html', (req, res) => {
  ...
  if (fs.existsSync(testHtmlPath)) {
    ...
    res.status(404).send('测试页面不存在，请先创建 test.html 文件')
  }
})
