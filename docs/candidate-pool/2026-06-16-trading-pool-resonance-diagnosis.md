...
1. 方向 B1（加权评分）vs B2（分层 AND）——哪种结构更容易理解和调参？
2. 方向 E（Jump hold 降权）的补偿阈值设为多少？（60%? 70%?）
3. 是否需要在 `rankTrendLiveStrategyConfig` 的三个预设中为这些新参数设置不同值？
4. 当前 `checkEntryConditions` 的 6 条件入口是否需要同步调整，还是只调交易池层？
