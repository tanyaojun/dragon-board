...
- 当前前端 `zlje / zljzb / cddje / cddjzb` 在 `moneyFlowSource = tdx_estimate` 时是估算口径，不是通达信官方 `L2_AMO` 口径。
- 通达信资金公式依赖 `L2_AMO(0..3, 0..3)`，即超大/大单/中单/小单在买入、卖出、暗盘等方向上的成交金额分档。
- mooTDX 当前可稳定提供的标准行情字段只有总主动买量、总主动卖量、成交额、成交量、OHLC 等，缺少 `L2_AMO` 真实分档金额。
- 因此估算公式可以让部分样本接近通达信结果，但无法保证逐票完全对齐，尤其是大单与中小单方向严重背离的股票。
- 十档盘口本身只提供价格档位和挂单量，不等同于 `L2_AMO`。如果只拿到十档深度，仍然需要真实逐笔/逐单或 L2 分档资金字段，才能稳定还原主力资金。
