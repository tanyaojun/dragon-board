...
1. 对每只股票收集包含当前帧且带帧身份的完整关注度 `ranks/percentiles`，检测本轮 Jump。横截面基准固定取“当前帧及其前 3 个市场帧”；只保留四帧全部存在、样本质量为 `ok/degraded` 的股票，至少 20 只才计算短周期百分位变化的横截面中位数。中位数在同一截面内共享，不是逐股字段。
