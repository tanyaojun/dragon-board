...
- [完成] 19. 按股票容量上限实现两阶段预警/确认：6 开头 10000 手、其它代码 6000 手，刷新周期 3 秒；清理图表“数据陈旧”提示
- [完成] 20. 关闭 shadow，正式启用 `prepend-logical`；交易时段 Longhu/THS TTL 调整为 3 秒，Longhu stale 请求等待一次有界增量并在失败时回退旧快照
