...
基础行情和分时量价固定使用腾讯代理，不接入 `python-bridge`。原因是腾讯分钟接口能一次返回当日完整分钟历史，程序中途启动也可补齐此前区间；`python-bridge` 当前只提供订阅后的实时 L1 快照，不能完整替代分钟历史和量比字段。
...
- 主图区左侧预留价格轴，右侧预留对应涨跌幅轴；两侧标签共用同一水平网格，以腾讯昨收价建立价格/涨幅映射。纵轴范围必须包含 `0%` 和两条均价线的全部有效值，向上下各留 5% 视觉余量；有效范围不足 `2%` 时按 `2%` 展开。腾讯昨收不可用时，允许由现价和当前涨幅反推；两者均不可用时右侧涨幅轴显示 `-`，左侧仍按有效价格范围绘制。
...
本轮增量只修改 THSBigOrder 的纯聚合、图表绘制、邻近测试和本专题文档；腾讯与同花顺现有代理合同已经提供所需字段，不新增代理路由。
