...
- THSBigOrder 自动化测试：34/34 PASS，退出码 0。覆盖四路原始/代理解析、直连成功不碰代理、单路独立降级、合法空涨停池不降级、同股票逐路 stale、首次失败不得伪造 stale、逐笔累计均价事件、金额/买卖过滤、秒级时间轴、旧 `DrawSignals` 清理和 MainForm 联动。
- Release 构建：0 warnings，0 errors。
- 真实上游探测：同花顺大单、新浪基础行情、腾讯分钟分时、同花顺涨停池均返回 HTTP 200。
- 生产 provider 实测股票 `002297`：状态 `直连`，四路 transport 均为 `Direct`，取得 1263 笔大单、242 个分钟点，名称“博云新材”、现价 28.36；本机 3000 端口虽有用户代理进程，但未触发代理降级。
