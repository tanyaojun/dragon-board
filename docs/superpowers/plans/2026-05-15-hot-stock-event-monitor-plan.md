...
**Goal:** 一次性完成选股通个股异动事件接入、热榜股票池过滤、盘中监控面板展示和测试验证。

**Architecture:** 通过 `proxy-server` 代理选股通 `event/history` 个股事件，根前端新增 `src/services/hotlist/**` service 负责解析、去重、热榜池过滤和订阅状态，新增 Vue 面板采用原异动提醒 HTML 的窄列表事件流形态，并复用项目主题变量和 `usePanel` 浮层行为接入主工作台。`DataLayer` 只作为当前热榜股票池和龙头候选的只读来源，不承载第三方 API 调用。
...
- 只监控个股异动事件，不接入板块拉升/板块跳水 `11000`、`11001`。
- UI 面板参照用户提供的异动提醒截图，采用窄列表事件流、顶部搜索、分类按钮、可折叠筛选和事件卡片；不套用 `SectorPanel.vue` 的题材大卡片布局。
...
- `/api/xuangubao/events` 返回个股异动事件，不包含 `11000`、`11001`。
- feed 能把选股通原始事件解析为统一 `HotStockAbnormalEvent`。
- monitor 只保留当前 `DataLayer.getStocks()` 热榜池命中的个股事件。
- monitor 能标记 DragonReview 候选命中。
- 同一事件重复刷新不重复展示。
- 接口失败时保留旧事件并暴露结构化错误。
- UI 面板可手动刷新、筛选事件类型、筛选候选命中、搜索股票，展示最近事件。
...
- [ ] 写代理测试，覆盖默认个股 types、过滤板块 types、成功响应、失败降级。
- [ ] 实现 `/api/xuangubao/events`，默认 `count=100`，最大 `200`。
- [ ] 只允许个股事件 types：`10001,10005,10003,10007,10002,10006,10004,10008,10012,10014,10009,10010`。
- [ ] 注册路由并更新启动日志列表。
...
- [ ] 写 feed 测试，覆盖个股事件解析、板块事件忽略、缺字段回退。
- [ ] 写 monitor 测试，覆盖热榜池过滤、候选标记、当日过滤、去重、失败保留旧状态。
- [ ] 实现类型、事件映射、方向和严重级别。
...
- [ ] 按异动提醒截图风格实现窄面板：标题、搜索框、分类按钮、可折叠筛选、事件卡片列表和底部来源。
- [ ] 支持手动刷新、全部/涨停/跌停/拉升跳水/候选命中筛选、股票搜索。
- [ ] 事件卡片显示时间、类型、股票、代码、涨跌幅、价格、候选命中标记。
