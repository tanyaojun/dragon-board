...
3. Bridge：启用 `L2_PROVIDER=qmt`、`QMT_L2_ENABLED=1` 后，WebSocket 输出 `money_flow_patch`、`l2_status` 和 QMT 十档盘口
...
- QMT 返回未知字段时，probe 返回 `field_mismatch`，不把全 0 数据伪装成 L2
- QuantBoard 正式资金流回测默认不接受 `estimated_l1`

## 已修复的启动保护

- 平台热榜命中缓存但 DataLayer 为空时，会重新水合并合并热榜数据，避免界面保持“股票 0”。
- 平台加载成功后会清理 loading 状态，避免后续非 force 刷新被短路。
