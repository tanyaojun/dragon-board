...
- `09:24` 只记录内部临门量价基线，不作为独立异动或语音依据。
- `09:25` 同时比较 `09:20 -> 09:25` 总量价和 `09:24 -> 09:25` 临门量价，输出候选成立或失败；候选可展示但不语音。
- `09:30` 比较临门基线和开盘第一窗口是否明显改善、未继续杀破竞价价且成交额承接；只有兼容阶段 `gapAlert` 可播。
- `09:35` 判断承接是否延续、是否守住开盘承接区并继续优于临门竞价涨幅；只有 `trendConfirm` 可播。
