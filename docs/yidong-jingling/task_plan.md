...
`09:24` 作为内部临门量价基线恢复，只参与 `09:25` 候选判断，不作为独立输出点。`09:25` 候选可展示不播，`10:00` 只是可选状态更新，不影响 `09:30/09:35` 播报。
...
| 竞价弱转强 | 已收敛到五时间点黑盒合同；`09:24` 仅作为内部临门量价基线。 |
...
2026-05 至 2026-06 初的旧计划包含模式族评分、前弱上下文、`auction_gap_delayed_board`、`15:00` 延迟确认、`dryRun`、质量门禁、流动性分层、proxy 授权播报和 `09:35-10:00` 状态机等探索。这些内容已废弃，不再作为待办或需求来源；`09:24` 临门基线只恢复为当前简化主链的量价辅助基线。如需考古，请使用 Git 历史。
