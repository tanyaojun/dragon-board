...
- 已核对当前 marker 实现、Type 语义、Longhu 归档结构与本地样本规模。
- 已确认当前规则缺少价格响应、净订单流、连续性、吸收效率和回撤验证；本地单日候选股样本不足以直接标定生产阈值。
- 下一步检索公开微观结构研究和行业实践，形成 2 至 3 套可计算方案及推荐口径，先交用户确认，不直接修改代码。
- 已通过 Crossref/arXiv 核对 OFI、订单流长记忆、订单簿韧性和流量毒性研究；证据支持“方向失衡 + 连续性 + 价格冲击/恢复”的组合口径，不支持仅凭固定金额或单一 Type 定性。
- 已对本地单日归档重放旧规则与两组连续事件规则；旧规则信号数量高出一个数量级。首版全参数扫描因 O(n²) 超时，已记录并改为后续按秒聚合实现。
- 已完成前 8 只股票的 THS/Longhu 同日逐笔映射交叉核验：匹配样本一致率 99.3%，后续仍需扩大日期和处理重复键。
- 用户已批准按推荐的高置信事件归因方案继续。恢复现场后确认语音批量文本和 marker 归属已完成，但生产 `CalculateMarkers` 仍是旧算法；当前归档扩为 17 只，阈值统计需要重新执行。
- 扩展样本重放首版因归档中存在非 6 列记录失败；已将显式清洗和丢弃计数列为算法输入合同，下一步定位异常行后重跑。
- 已完成扩展重放：17 只、95,274 条原始、95,150 条有效、124 条无效占位；旧规则 1,153/1,156，高置信固定 500 万规则 25/12。
- 已把批准方案写入 `big-order-event-attribution-design.md`，补齐自适应阈值、事件窗口、买活跃、承接好、延迟确认语音、代码边界和验收合同；当前等待书面设计复核后再写实施计划和业务代码。
- 已按用户要求实测 THS/Longhu 历史日期能力。Longhu `Date`/`TradeDate` 和 THS `date` 均未改变返回的权威交易日，当前两个上游只提供最近交易日逐笔；主界面日期筛选不实施，避免日期错配。
- 本地 gzip 归档只能回放已经采集的日期，不能补抓未归档历史。可行路径仍是从现在持续归档，或另行寻找具有明确历史逐笔合同的数据源。
- 用户确认以 7/17 暴跌日作为当前分析基准并继续实施；新增样本重算为 28 只、169,870 条原始、169,623 条有效，高置信固定 500 万规则为 31 次点火/25 次砸盘。
- 已写入 `big-order-event-attribution-implementation-plan.md`，执行范围限定为 detector、provider 委托、marker tracker、C# 测试和专题文档。
- 已完成事件 detector、provider 委托和 marker tracker 的 RED/GREEN：单笔拒绝、连续性、方向纯度、价格冲击/保留、P90、买活跃、承接好、确认窗口关闭和延迟语音合同均有自动化测试。
- 生产 Release 产物重放 28 只归档得到 24 点火、23 砸盘、5 买活跃、0 承接好；承接为 0 是因为价格恢复率均未达到 50%，不是卖压或吸收分支未执行。
- 全样本 detector 最终合计 997ms、最慢单股 363ms，输出与优化前一致。
- 最终验证：C# runner 77/77；THSBigOrder Release 构建 0 warning / 0 error；生产重放 24 点火、23 砸盘、5 买活跃、0 承接好；`git diff --check` 通过。下一交易日仍需人工核对盘中延迟确认语音和误报/漏报。
