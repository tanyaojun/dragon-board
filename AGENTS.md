...
- 设计、实现、调试或审查数据采集、持久化、同步、迁移、缓存、读取、查询、回放、分析和界面观察链路时，必须先使用项目级 `skills/fail-visible-data-pipelines/SKILL.md`。
- 所有数据采集、清洗、快照写入、同步、迁移、导入和缓存链路都不得因质量判断、字段缺失、覆盖率不足、非标代码、时间乱序或部分来源失败而主动阻断已有数据进入持久化链路。
- 唯一允许不写入的情形是：本轮所有数据源均不可用，因而没有任何可保存的原始记录。除此之外必须保存原始数据，并在记录或 frame 上保留完整质量标记、来源状态和结构化原因。
- 数据质量检查的职责是主动暴露、聚合、上报和展示问题，供用户观察、反馈、补采和下游决策；不得把问题静默转化为“没有数据”“全零结果”或不可恢复的数据丢失。
- 数据质量问题同样不得构成读取、查询、回放或界面展示的硬门禁。只要存在任何可用数据，就必须向用户展示原始数据、可计算部分、缺失位置和结构化原因；可以降低可信度或停产分析结论，但不得整段返回空结果、隐藏已有数据或阻止用户查看。
- 任何面向用户的观察/分析层遇到输入异常，必须保留可诊断状态和结构化原因。分析结论可以标记为不可靠，但不得覆盖、删除或阻止原始数据。
- 新增或修改数据链路时，测试必须证明：部分数据异常仍会保存并带标记；异常原因可由公开 API 或界面诊断；仅全源无数据时才返回采集失败。
...
- 数据质量异常时必须返回原始数据、可计算部分和结构化原因；不得静默吞掉，不得用 `0`、空结果、单帧或其他评分伪装缺失。派生交易结论可以停产或标记为不可靠，但不得连带阻断数据保存、读取、回放和界面观察。
