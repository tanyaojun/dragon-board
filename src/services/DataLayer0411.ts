...
class DataLayer {
  ...
  private buildStockSnapshot(stock: MergedStock, rank: number) {
    return {
      code: stock.code,
      name: stock.name,
      rank,
      ...
      firstSeen: stock.firstSeen,
      lastSeen: stock.lastSeen,
      platformName: stock.platformName,
      ...
      themes:
        stock.themes?.slice(0, 10).map((t: any) => ({
          id: t?.id,
          name: t?.name,
  ...
  private buildSectorSnapshot(block: JxbkBlockData) {
    return {
      code: block.code,
      name: block.name,
  ...
  private buildHotThemeSnapshot(theme: any, rank: number) {
    const themeId = theme?.id || theme?.themeId || theme?.code || theme?.name
    const metrics = themeId ? this.getThemeMetrics(themeId) : undefined
    const correlation = themeId ? this.getThemeCorrelation(themeId) : undefined

    return {
      rank,
      id: themeId,
      name: theme?.name || '',
      heatScore: theme?.heatScore ?? metrics?.heatScore ?? 0,
      heatLevel: theme?.heatLevel ?? metrics?.heatLevel ?? '',
      momentum: theme?.momentum ?? metrics?.momentum ?? 0,
      trend: theme?.trend ?? metrics?.trend ?? 0,
      acceleration: theme?.acceleration ?? metrics?.acceleration ?? 0,
      correlation: theme?.correlation ?? metrics?.correlation ?? 0,
      stockCount: theme?.stockCount ?? metrics?.stats?.stockCount ?? 0,
      ztCount: theme?.ztCount ?? metrics?.stats?.ztCount ?? 0,
      leaderCount: theme?.leaderCount ?? metrics?.stats?.leaderCount ?? 0,
      relatedThemes:
        metrics?.relatedThemes?.slice(0, 10).map((item: any) => ({
          id: item?.id || item?.themeId || item?.code || item?.name,
          name: item?.name || '',
          correlation: item?.correlation ?? item?.score ?? 0,
        })) || [],
      jxbk: {
      ...
      correlationSummary: correlation
        ? {
            stockCount: correlation.stockCount ?? 0,
            avgChange: correlation.avgChange ?? 0,
            leaderCount: correlation.leaderCount ?? 0,
          }
        : null,
    }
  }

  private buildMarketSnapshot(marketData: DataState['analysis']['breath']['marketData']) {
    return {
      ...
      zhaban: {
        ...
        ztCount: marketData?.zhaban?.ztCount || 0,
      },
      continuousBoards: {
      ...
      yesterdayLimit: {
        ...
        ztCount: marketData?.yesterdayLimit?.ztCount || 0,
  ...
  private buildMoneyFlowSnapshot(marketData: DataState['analysis']['breath']['marketData']) {
    return {
      main: marketData?.moneyFlow?.main || 0,
      retail: marketData?.moneyFlow?.retail || 0,
      cddje: marketData?.cddje || 0,
      cddjzb: marketData?.cddjzb || 0,
      yesterdayZtPerformance: marketData?.yesterdayZtPerformance || 0,
    }
  }

  private buildAlertsSnapshot(limit = 50) {
    const stats = this.state.analysis.alerts.stats
    return {
      stats: {
        total: stats?.total || 0,
      ...
      items: this.getAlerts(limit).map((alert) => ({
        id: alert.id,
        code: alert.code,
        name: alert.name,
        type: alert.type,
        ...
        readTime: alert.readTime,
      })),
    }
  }

  private buildAlgorithmSnapshot(limit = 100) {
    const results = Array.from(this.state.analysis.algorithm.results.entries())
      .slice(0, limit)
      .map(([code, result]) => ({
        code,
        result,
    ...
    return {
      config: this.state.analysis.algorithm.config,
      results,
      lastUpdate: this.state.analysis.algorithm.lastUpdate,
    }
  }

  private buildRankHistorySnapshot(limit = 200) {
    return {
      snapshotDate: this.state.rankHistory.snapshotDate,
      lastUpdate: this.state.rankHistory.lastUpdate,
      items: Array.from(this.state.rankHistory.byCode.entries())
        .slice(0, limit)
        .map(([code, rank]) => ({ code, rank })),
    }
  }

  private triggerSnapshotSave(
    snapshotType: 'quarter' | 'half' | 'hour' | 'daily',
    key: string,
    ...
    const field = keyMap[snapshotType]
    if (this.lastSnapshotKey[field] === key) return
    this.lastSnapshotKey[field] = key

    saveFn().catch((error) => {
      console.error(`[DataLayer] ${snapshotType}快照自动保存失败:`, error)
    })
  }
  ...
  async saveQuarterHourSnapshot(snapshotTime?: Date): Promise<boolean> {
    try {
      ...
      const snapshot = {
        ...
        hotlist: stocks.map((stock, index) => this.buildStockSnapshot(stock, index + 1)),

        sectors: jxbkBlocks.map((block) => this.buildSectorSnapshot(block)),
        hotThemes: this.getHotThemes()
          .slice(0, 20)
          .map((theme: any, index: number) => this.buildHotThemeSnapshot(theme, index + 1)),
        ...
        moneyFlow: this.buildMoneyFlowSnapshot(marketData),
        marketStats: this.buildMarketSnapshot(marketData),
        alerts: this.buildAlertsSnapshot(30),
        algorithm: this.buildAlgorithmSnapshot(50),
        rankHistory: this.buildRankHistorySnapshot(100),
        rotation: this.state.analysis.rotation?.current || null,
      }
  ...
  async saveHalfHourSnapshot(snapshotTime?: Date): Promise<boolean> {
    try {
      ...
      const snapshot = {
        ...
        moneyFlow: this.buildMoneyFlowSnapshot(marketData),
        marketStats: this.buildMarketSnapshot(marketData),
        alerts: this.buildAlertsSnapshot(50),
        algorithm: this.buildAlgorithmSnapshot(80),
        rankHistory: this.buildRankHistorySnapshot(150),
        rotation: this.state.analysis.rotation?.current || null,
      }
  ...
  async saveHourlySnapshot(snapshotTime?: Date): Promise<boolean> {
    try {
      ...
      const snapshot = {
        ...
        hotlist: stocks
          .slice(0, 100)
          .map((stock, index) => this.buildStockSnapshot(stock, index + 1)),
        ...
        marketStats: this.buildMarketSnapshot(marketData),
        zhaban: this.buildMarketSnapshot(marketData).zhaban,
        moneyFlow: this.buildMoneyFlowSnapshot(marketData),
        continuousBoards: this.buildMarketSnapshot(marketData).continuousBoards,
        alerts: this.buildAlertsSnapshot(80),
        algorithm: this.buildAlgorithmSnapshot(100),
        rankHistory: this.buildRankHistorySnapshot(200),
        rotation: this.state.analysis.rotation?.current || null,
      }

      const cleanSnapshot = JSON.parse(JSON.stringify(snapshot))
      await this.saveToIndexedDB(cleanSnapshot)
      console.log(`[DataLayer] ✅ 日级快照已保存: ${key}`)
      return true
    } catch (error) {
  ...
  generateDailySnapshot(): any {
    ...
    const topThemes = hotThemes
      .slice(0, 20)
      .map((t: any, i: number) => this.buildHotThemeSnapshot(t, i + 1))

    // ========== 11. 热榜股票 ==========
    const hotlist = [...allStocks]
      .sort((a, b) => (a.compRank || 999) - (b.compRank || 999))
      .map((s, index) => this.buildStockSnapshot(s, s.compRank || index + 1))
    ...
    return {
      ...
      // 轮动分析
      rotation,
      alerts: this.buildAlertsSnapshot(100),
      algorithm: this.buildAlgorithmSnapshot(150),
      rankHistory: this.buildRankHistorySnapshot(300),
  ...
  async saveDailySnapshot(snapshotTime?: Date): Promise<boolean> {
    try {
      ...
      const cleanSnapshot = JSON.parse(JSON.stringify(snapshot))
      await this.saveToIndexedDB(cleanSnapshot)
      console.log(`[DataLayer] ✅ 整点快照已保存: ${key}`)
      return true
    } catch (error) {
  ...
  startTimer() {
    this.timer = setInterval(() => {
      ...
      // 15:00 日级快照允许优先触发（无论是否在交易时间）
      if (h === 15 && m === 0) {
        this.triggerSnapshotSave('daily', key, () => this.saveDailySnapshot(now))
        return // 日级快照优先级最高
      }
      ...
      if (m === 0) {
        this.triggerSnapshotSave('hour', key, () => this.saveHourlySnapshot(now))
        return // 整点优先级高于半点
      }

      // 半点快照
      if (m === 30) {
        this.triggerSnapshotSave('half', key, () => this.saveHalfHourSnapshot(now))
        return // 半点优先级高于一刻
      }

      // 一刻快照
      if (m === 15 || m === 45) {
        this.triggerSnapshotSave('quarter', key, () => this.saveQuarterHourSnapshot(now))
      }
    }, 1000)
  }
