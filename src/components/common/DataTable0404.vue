...
const getRowTitle = (stock: Stock) => {
  ...
  if (stock.rankChange !== undefined && stock.rankChange !== 0) {
    ...
    titles.push(`${stock.rankChange > 0 ? '🔴' : '🟢'} 排名变化: ${stock.rankChange > 0 ? '+' : ''}${change}%`)
  }

  // MACD 信号
  if (stock.macdCross === 'golden') {
    titles.push(`🔱 MACD 金叉`)
  } else if (stock.macdCross === 'death') {
    titles.push(`💀 MACD 死叉`)
  }
  ...
  titles.push(`   📈 方向一致性: ${directionText} (${stock.directionConfidence || 0}%)`)

  // 动量加速度
  const accelerationText = stock.accelerationSignal === 'buy' ? '买入' : stock.accelerationSignal === 'sell' ? '卖出' : '持有'
  titles.push(`   ⚡ 动量加速度: ${accelerationText} (${stock.accelerationConfidence || 0}%)`)

  // 零线交叉
  const crossText = stock.crossSignal === 'buy' ? '买入' : stock.crossSignal === 'sell' ? '卖出' : '持有'
  titles.push(`   🔄 零线交叉: ${crossText} (${stock.crossConfidence || 0}%)`)

  // 最终信号
  if (stock.finalSignal && stock.finalSignal !== 'none') {
    const signalText = stock.finalSignal === 'buy' ? '买入' : stock.finalSignal === 'sell' ? '卖出' : '持有'
    titles.push(`\n✨ 综合判断: ${signalText} (${Math.round(stock.finalConfidence || 0)}%)`)
  }
...
const getConfidenceTitle = (stock: any) => {
  ...
  let title = `🎯 综合判断: ${signalText} (置信度: ${confidence}%)\n`
  title += '─'.repeat(30) + '\n'
...
// 判断是否显示金叉死叉标记（早盘9:30-10:00 和尾盘14:30-15:00）
const shouldShowMacdSignal = (stock: any) => {
  ...
  // 早盘 9:30-10:00
  const isMorning = (hour === 9 && minute >= 30) || (hour === 10 && minute === 0)
  // 尾盘 14:30-15:00
  const isAfternoon = (hour === 14 && minute >= 30) || (hour === 15 && minute === 0)
...
const openRankTrendPanel = (event?: MouseEvent) => {
  if (event) {
    rankTrendPanelTriggerRect.value = (event.currentTarget as HTMLElement).getBoundingClientRect()
  }
...
const openRankTrendForStock = () => {
  if (contextMenu.value.stock) {
    ...
    setTimeout(() => {
      ...
      if (searchInput) {
        searchInput.value = contextMenu.value.stock.code
