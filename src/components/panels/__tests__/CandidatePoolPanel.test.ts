...
describe('CandidatePoolPanel source contract', () => {
  ...
  test('exposes trading-pool as a candidate projection without historical trade-log semantics', () => {
    const source = panelSource()
    const tabIndex = source.indexOf('data-testid="candidate-pool-tab-trading"')
    const tradingViewIndex = source.indexOf('activePoolTab === \'trading\'')

    expect(tabIndex).toBeGreaterThan(0)
    expect(tradingViewIndex).toBeGreaterThan(tabIndex)
    expect(source).toContain('候选池 thesis 记录的买点复筛投影，不写入历史交易日志')
    expect(source).toContain('刷新交易池')
    expect(source).toContain('暂无交易池投影')
    expect(source).toContain('tradingPoolStats')
    expect(source).toContain('visibleTradingPoolRows')
    expect(source).toContain('评分')
    expect(source).toContain('综合')
    expect(source).toContain('票数')
    expect(source).toContain('风险')
    expect(source).toContain('来源')
    expect(source).toContain('tradingPoolSourceLabel')
    expect(source).toContain('formatRiskFlags')
    expect(source).toContain('观察买点')
    expect(source).toContain('已退出')
    expect(source).toContain('信号过期')
    expect(source).toContain('persistedTradingPoolEntries')
    expect(source).toContain('listTradingPoolEntries')
    expect(source).toContain('createTradingPoolEntry')
    expect(source).toContain('updateTradingPoolEntry')
    expect(source).toContain('readTradingPoolSnapshot')
    expect(source).toContain('tradingPoolEntryForCandidate')
    expect(source).toMatch(/const thesisCandidates = candidates\.value/)
    expect(source).toContain('liveStocks')
    expect(source).toContain('dataLayer.getStocks()')
    expect(source).toMatch(/row\.signalSnapshot\.jumpConfidence/)
    expect(source).toMatch(/row\.signalSnapshot\.finalConfidence/)
    expect(source).toMatch(/row\.scoringBreakdown\?\.totalScore/)
    expect(source).toContain('评分拆解')
    expect(source).toContain('交易池动作')
    expect(source).toMatch(/row\.signalSnapshot\.buyVotes/)
    expect(source).toMatch(/row\.signalSnapshot\.macdCross/)
    expect(source).not.toContain('强共振条件')
    expect(source).not.toContain('8 通过')
    expect(source).not.toContain('共振评分')
    expect(source).not.toContain('consensus-matrix')
    expect(source).not.toContain('consensus-checks')
    expect(source).not.toContain('trade_type=trading_pool')
    expect(source).not.toContain('entry/exit')
    expect(source).not.toMatch(/watch\(\s*candidates[\s\S]*deep:\s*true/)
    expect(source).toContain('持仓观察需要真实持仓/成交数据后再接入')
  })
})
